 ...
 *
 * Return: Pointer to the allocated array, or NULL on allocation failure or if @num is 0.
 */
 ...
 *
 * Calculates the write amplification factor based on the provided
 * over-provisioning value. The formula used is:
 *     WAF = 0.5 / over_provisioning + 0.7
 *
...
/**
 * compute_validity - Calculate the validity metric based on Write Amplification Factor (WAF).
 * @waf: The Write Amplification Factor, must be greater than 1.0.
 *
 * This function computes a validity value as a function of the Write Amplification Factor (WAF).
 * The validity is defined as 1.0 - (1.0 / waf), representing the proportion of valid data
 * after accounting for write amplification effects.
 *
 * Return: The computed validity as a double.
 */
...
static void dprint_invalidation(const struct sprandom_info *info)
{
	...
	dprint(FD_SPRANDOM, "Invalidation[%d] %"PRIu64" %zu %.04f %d\n",
		info->current_region,
